...
void automaton::p1_deposit() {
  eosio::check(data.state == CREATED, "team1 funds already deposited");
  data.state = P1_DEPOSITED;
}

void automaton::p2_deposit() {
  eosio::check(data.state == P1_DEPOSITED, "team2 cannot deposit now");
...
void automaton::reveal(bool is_team1,
